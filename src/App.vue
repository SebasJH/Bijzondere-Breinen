<template>
    <div class="container mx-auto">
        <div class="mx-4 sm:mx-8">
            ...
            <h1 class="text-4xl mb-8 mt-20 font-semibold">Bijzondere Breinen V4</h1>
                ...
                <div v-for="(item, index) in newArr" :key="index">
                    <div class="grid gap-3 grid-cols-footer grid-flow-col-dense text-white text-center mb-3">
                        <template v-for="category in item" :key="category.text">
                            ...
                            <!-- Input -->
                            <input type="image" onerror="this.style.display='none'" v-for="item in category.items" :key="item" :id="`${category.text}-${item.title}`" src="/" alt="" draggable="false" class="col-start-3 bg-gray-400 hover:bg-gray-500 cursor-pointer focus:ring-4 ring-black outline-none" @click="select($event)"/>
                        </template>
                    </div>
                </div>
            </div>
